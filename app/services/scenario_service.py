...
def _build_auth_features(elements: list[UIElement]) -> tuple[list[dict], set[str]]:
    ...
    # NOTE (demande explicite) : "Inscription" et les boutons OAuth
    # (Google/Microsoft/...) ne doivent JAMAIS produire de scénario de
    # test — trop instables/risqués à automatiser (création de vrais
    # comptes tiers, redirections externes hors contrôle du script). Les
    # labels correspondants restent dans `consumed` (voir boucle
    # ci-dessus) pour ne PAS non plus réapparaître comme scénario
    # générique "cliquer + vérifier changement de page" côté LLM/nav.
    # Seul le login natif (formulaire email/mot de passe de CE site) est
    # encore testé, et seulement s'il existe réellement (login_els).

    if login_els:
        trigger_label = login_els[0].label
        features.append({
            "name": "Connexion",
            "description": f"Se connecter via « {_display_label(trigger_label)} ».",
            "elements": [trigger_label],
            "priority": "high",
            "_template": True,
            "_el_type": "auth_login",
