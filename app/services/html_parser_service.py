...
def _extract_site_title(html_code: str) -> str | None:
    """
    Extrait le contenu de la balise <title> du HTML source, pour l'afficher
    tel quel comme en-tête avant la liste des scénarios (voir App.jsx,
    panel-header "Tous les scénarios"). Distinct de `page_type`/
    `raw_description` qui restent des résumés internes générés par le LLM :
    ici on veut le VRAI titre du site tel qu'écrit par le développeur de
    la page testée, sans reformulation.
    """
    try:
        soup = BeautifulSoup(html_code, "html.parser")
...
async def analyze_html_code(html_code: str) -> UIAnalysisResult:
    site_title = _extract_site_title(html_code)
